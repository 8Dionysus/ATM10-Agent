...
- Observer pilot runtime slice: `scripts/pilot_runtime_loop.py` + `scripts/pilot_turn_smoke.py`, with local `F8` push-to-talk, Windows `monitor|region` screen capture, `Whisper GenAI -> OpenVINO VLM -> gateway hybrid_query(profile=combo_a) -> OpenVINO grounded reply -> tts_runtime_service` as the default live path, additive ATM10 evidence artifacts `atm10_session_probe_v1` and `live_hud_state_v1`, short player-facing replies with Russian default language policy, and resilient degraded behavior when `combo_a` grounding or live TTS engines are unavailable, including Windows-host audible fallback before last-resort silent audio.
