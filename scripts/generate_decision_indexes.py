...
DECISION_PREFIX = "ATM10-D"
DECISION_RE = re.compile(r"^ATM10-D-(\d{4})-[a-z0-9][a-z0-9-]*\.md$")
DECISION_ID_RE = re.compile(r"^ATM10-D-\d{4}$")
...
INDEX_PATHS = (
    ...
    INDEX_DIR / "by-companion-layer.md",
    INDEX_DIR / "by-operator-surface.md",
...
def collect_decision_records(repo_root: Path = REPO_ROOT) -> tuple[list[DecisionRecord], list[tuple[str, str]]]:
    ...
    for path in sorted(decision_root.glob("*.md")):
        ...
        if not path.name.startswith(f"{DECISION_PREFIX}-"):
            issues.append((rel, "top-level decision markdown must use ATM10-D-####-slug.md"))

    for path in sorted(decision_root.glob(f"{DECISION_PREFIX}-*.md")):
        rel = path.relative_to(repo_root).as_posix()
        match = DECISION_RE.match(path.name)
        if not match:
            issues.append((rel, "filename must match ATM10-D-####-slug.md"))
            continue
        ...
        companion_layers = _metadata_value(metadata, "Companion layers", rel, issues)
        operator_surfaces = _metadata_value(metadata, "Operator surfaces", rel, issues)
        ...
        if decision_id and not DECISION_ID_RE.match(decision_id):
            issues.append((rel, "Decision ID must match ATM10-D-####"))
        if original_date:
            try:
        ...
        if decision_id and original_date and title:
            records.append(
                DecisionRecord(
                    ...
                    companion_layers=_split_values(companion_layers),
                    operator_surfaces=_split_values(operator_surfaces),
    ...
    if numbers:
        ...
        if numbers != expected_numbers:
            issues.append((DECISIONS_DIR.as_posix(), "decision numbers must be contiguous from ATM10-D-0001"))
...
def validate_index_contract(repo_root: Path = REPO_ROOT) -> list[tuple[str, str]]:
    ...
    text = path.read_text(encoding="utf-8")
    required = [
        "schema_version: atm10_decision_index_contract_v1",
        "authority: docs/decisions/AGENTS.md",
        "source_records: docs/decisions/ATM10-D-*.md",
...
def _table(records: list[DecisionRecord], *, columns: tuple[str, ...]) -> str:
    ...
    for record in records:
        ...
        for column in columns:
            ...
            elif column == "Companion layers":
                values.append(", ".join(record.companion_layers))
            elif column == "Operator surfaces":
                values.append(", ".join(record.operator_surfaces))
...
def render_index_files(records: list[DecisionRecord]) -> dict[Path, str]:
    ...
    return {
        ...
        INDEX_DIR / "by-companion-layer.md": _grouped_index(
            "Decisions By Companion Layer",
            _group_by(records, "companion_layers"),
        ),
        INDEX_DIR / "by-operator-surface.md": _grouped_index(
            "Decisions By Operator Surface",
            _group_by(records, "operator_surfaces"),
        ),
