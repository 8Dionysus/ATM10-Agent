...
3. Optional HTTP auth:
   gateway/voice/tts support `--service-token` and `ATM10_SERVICE_TOKEN`; when a token is configured, `X-ATM10-Token` is mandatory; the gateway unauthorized path returns `error_code=unauthorized` and HTTP 401.
