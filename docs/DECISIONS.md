...
* Для Wave 7 Turbo policy thresholds централизованы в `src/agent_core/ops_policy.py` (`readiness_window=14`, `required_baseline_count=5`, `max_warn_ratio=0.20`, `required_ready_streak=3`, `source_stale_after_hours=36`) и используются readiness/governance/progress/transition checker-скриптами.
* В `pytest` smoke workflow добавлен report-layer для контрактов: `scripts/validate_ops_contracts.py` (`validation_summary_v1`) и `scripts/build_ops_contract_index.py` (`ops_contract_index_v1`) с публикацией в step summary/artifacts (`runs/ci-ops`), без включения fail-gate в текущей итерации.
...
* Streamlit `Latest Metrics` upgraded до `Ops Readiness`: учитывается optional source `transition_summary.json`, показываются `allow_switch` и per-source freshness (`fresh|stale|missing|unknown|invalid`); smoke contract расширен полями `ops_readiness_contract_ok|ops_readiness_missing_fields|ops_readiness_warnings|ops_readiness_snapshot` (aditive-only).
* Для операционного bootstrap `G2` добавлен `scripts/bootstrap_gateway_sla_nightly_history.py`: локально прогоняет полный chain (`gateway_http_smoke -> sla -> trend -> readiness -> governance -> progress -> transition`) и пишет `gateway_sla_bootstrap_summary_v1` в `runs/nightly-gateway-sla-bootstrap/*`.
* Bootstrap policy зафиксирован как `signal-first/no synthetic promotion`: локальный bootstrap валидирует связность пайплайна, а разрешение switch на nightly strict (`fail_nightly`) принимается только по реальным nightly snapshots в `runs/nightly-gateway-sla-*`.
