...
* After the successful `qwen3-vl` custom export, local HF caches were cleaned (`models/hf_cache`, `models/hf_raw/qwen3-vl-4b`, `C:\Users\Admin\.cache\huggingface`) to free disk space, with the explicit understanding that future export/inference scenarios would require re-downloading weights.
