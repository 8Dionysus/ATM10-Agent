...
* (`release_only`) Для Wave 7 Turbo policy thresholds централизованы в `src/agent_core/ops_policy.py` (`readiness_window=14`, `required_baseline_count=5`, `max_warn_ratio=0.20`, `required_ready_streak=3`, `source_stale_after_hours=36`) и используются readiness/governance/progress/transition checker-скриптами.
* (`release_only`) В `pytest` smoke workflow добавлен report-layer для контрактов: `scripts/validate_ops_contracts.py` (`validation_summary_v1`) и `scripts/build_ops_contract_index.py` (`ops_contract_index_v1`) с публикацией в step summary/artifacts (`runs/ci-ops`), без включения fail-gate в текущей итерации.
* (`release_only`) Для `G2.3` добавлен transition gate `scripts/check_gateway_sla_fail_nightly_transition.py` (`gateway_sla_fail_nightly_transition_v1`): `allow_switch=true` только при одновременном выполнении readiness/governance/progress baseline условий; nightly strict trend step (`critical_policy=fail_nightly`) запускается только при `allow_switch=true`.
* (`release_only`) Streamlit `Latest Metrics` upgraded до `Ops Readiness`: учитывается optional source `transition_summary.json`, показываются `allow_switch` и per-source freshness (`fresh|stale|missing|unknown|invalid`); smoke contract расширен полями `ops_readiness_contract_ok|ops_readiness_missing_fields|ops_readiness_warnings|ops_readiness_snapshot` (aditive-only).
* (`release_only`) Для операционного bootstrap `G2` добавлен `scripts/bootstrap_gateway_sla_nightly_history.py`: локально прогоняет полный chain (`gateway_http_smoke -> sla -> trend -> readiness -> governance -> progress -> transition`) и пишет `gateway_sla_bootstrap_summary_v1` в `runs/nightly-gateway-sla-bootstrap/*`.
* (`release_only`) Bootstrap policy зафиксирован как `signal-first/no synthetic promotion`: локальный bootstrap валидирует связность пайплайна, а разрешение switch на nightly strict (`fail_nightly`) принимается только по реальным nightly snapshots в `runs/nightly-gateway-sla-*`.
* Для operating mode `solo+AI` принят manual dispatch подход для `Gateway SLA Readiness Nightly`: запуск через GitHub Actions UI (`workflow_dispatch`, default branch репозитория; на `2026-03-02` это `master`) без `gh` CLI как baseline surface.
* Для накопления real nightly history принят `calendar-day guardrail`: в switch evidence учитывается не более одного run на UTC-сутки; rerun в те же UTC-сутки разрешен только как recovery и не ускоряет progression к `allow_switch=true`.

## 2026-03-02 (master-truth docs sync)

* Источник операционной истины зафиксирован как `master` (baseline SHA: `e6e8f0f`): на default branch доступны `readiness + governance + progress`; `transition/bootstrap` отсутствуют.
* Для снятия doc drift принята явная branch-scope маркировка: `master_available`, `release_only`, `planned_resync`.
* Policy остается неизменной: `no synthetic promotion`; switch в strict-mode не принимается без real transition evidence в `master` (до `planned_resync` решение по `allow_switch` не применяется).
