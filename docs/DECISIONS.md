...
* Для `G2.manual` добавлен агрегирующий helper `scripts/check_gateway_sla_manual_cycle_summary.py` с контрактом `gateway_sla_manual_cycle_summary_v1`: единый operator-loop snapshot сводит preflight decision и текущий readiness/governance/progress/transition status без изменения CI/workflow и runtime API контрактов.
* Для локального solo+AI режима добавлен wrapper `scripts/run_gateway_sla_manual_nightly.py` с контрактом `gateway_sla_manual_nightly_runner_v1`: manual nightly-chain теперь выполняется без `workflow_dispatch`, с local artifact UTC guardrail (`1 accounted run/day`), recovery-mode без progression credit и fail-fast политикой.
* Для ежедневного operator-loop добавлен read-only helper `scripts/check_gateway_sla_manual_cadence_brief.py` с контрактом `gateway_sla_manual_cadence_brief_v1`: единая machine-readable сводка `attention_state + forecast(UTC ETA)` формализует решение `run now vs wait/recovery/repair`, не меняя runtime/API/CI и сохраняя `no-backfill` policy.
* Для execution-window `2026-03-03 -> 2026-03-17` принят режим `G2-only until go/no-go`: параллельные execution-track (`G3/G5`) откладываются до завершения gate-review.
* `G2 Conservative Gate` зафиксирован как policy `conservative`: strict switch допускается только при одновременном выполнении `window_observed>=14`, `critical_count=0`, `warn_ratio<=0.20`, `latest_ready_streak>=3`, `transition.allow_switch=true`.
* Surface policy зафиксирован как `nightly_only`: при потенциальном `GO` переключается только nightly strict path; `pytest.yml` остается `signal_only` до отдельного explicit решения.
* Baseline lock на `2026-03-03`: `pytest -q = 383 passed`, `progress.remaining_for_window=12`, `progress.remaining_for_streak=3`, `transition.allow_switch=false` и `decision_status=hold`; изменения runtime API/summary schema в рамках этого окна не планируются.
* По явному operator-запросу выполнен manual switch override: в `.github/workflows/gateway-sla-readiness-nightly.yml` strict step `gateway_sla_trend_snapshot --critical-policy fail_nightly` исполняется стабильно на каждом nightly run (без условия `allow_switch`).
