...
cd D:\atm10-agent
.\.venv\Scripts\Activate.ps1
python scripts/check_gateway_sla_fail_nightly_progress.py --readiness-runs-dir runs/nightly-gateway-sla-readiness --governance-runs-dir runs/nightly-gateway-sla-governance --readiness-history-limit 60 --governance-history-limit 60 --expected-readiness-window 14 --expected-required-baseline-count 5 --expected-max-warn-ratio 0.20 --required-ready-streak 3 --policy report_only --runs-dir runs/nightly-gateway-sla-progress --summary-json runs/nightly-gateway-sla-progress/progress_summary.json
```
...
* `release_only`: transition checker и conditional strict step есть в release-ветке.
* `planned_resync`: вернуть transition цепочку в `master` отдельным PR.
* `master_available`: до resync решение по `allow_switch` не вычисляется в default branch.
...
Nightly transition integration (`release_only`):
...
`master_available` note:

* На `origin/master` (baseline `e6e8f0f`) transition step отсутствует; `transition_summary.json` не является обязательным артефактом nightly-цикла.
...
* `release_only`: bootstrap entrypoint и summary contract доступны в release-ветке.
* `planned_resync`: вернуть bootstrap tooling в `master` отдельным PR.
* `master_available`: использовать только real-nightly accumulation без bootstrap evidence.
...
* `master_available`: readiness + governance + progress summaries.
* `release_only`: transition summary (`allow_switch`) и strict gate branch.
* `planned_resync`: вернуть transition coverage в default branch.
...
`release_only` (если transition summary присутствует):
...
* `runs/nightly-gateway-sla-transition/transition_summary.json` (`release_only`)

### Gate decision baseline (`release_only`, `planned_resync`)
...
* До возвращения transition step в `master` strict switch-решение не принимается; накопление истории продолжается по readiness/governance/progress.

### Failure handling

* Если transition summary не создан на `master`: это ожидаемо, rerun по этой причине не нужен.
* Если transition summary не создан в `release_only` run: rerun в тот же UTC-день только для восстановления chain.
...
* `gateway_sla_fail_nightly_transition_v1` (`release_only`)

UI поля progress-блока:

* `allow_switch` (`release_only`)
* `transition_decision_status` (`release_only`)
* `readiness_status`
* `latest_ready_streak`
* `decision_status`
* `remaining_for_window`
